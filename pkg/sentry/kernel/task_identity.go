...
func (t *Task) saveCreds() *auth.Credentials {
	return t.Credentials()
}

func (t *Task) loadCreds(creds *auth.Credentials) {
	t.creds.Store(creds)
}
...
func (t *Task) setKUIDsUncheckedLocked(newR, newE, newS auth.KUID) {
	creds := t.Credentials().Fork() // The credetials object is immutable. See doc for creds.
...
func (t *Task) setKGIDsUncheckedLocked(newR, newE, newS auth.KGID) {
	...
	creds = creds.Fork() // The credetials object is immutable. See doc for creds.
